...
double bds::GetMeanDistance(std::vector<boid> boid_vector) {
  ...
  /*for (lu_int i = 0; i < boid_vector.size(); ++i)
  {
      ...
      for (lu_int j = 0; j < boid_vector.size(); ++j)
      {
          ...
          { // ho messo un controllo vicini qua perchè non credo abbia senso
  considerare nel calcolo della media la distanza tra i boid megalontani (ma
  forse lo ha...) dist_sum += sqrt(squaresum(i_boid.pos() - j_boid.pos()));
              ++n_events; // perchè non so a priori quante sono le coppie di
  boids che contribuiscono alla determinazione della distanza media
          }
      }
  } */
  double meandist = dist_sum / n_events;
  return meandist;
}  // tira fuori la distanza media tra i boids di un dato vettore, da chiamare
   // nel ciclo
...
double bds::GetStdDevDistance(std::vector<boid> boid_vector) {
  ...
  square_dist_sum_2 = std::accumulate(boid_vector.begin(), boid_vector.end(),
  double{0}, [&] (double& d_s, boid& i_boid) { double local_sum =
  std::accumulate(boid_vector.begin(), boid_vector.end(), double{0.}, [&]
  (double& ds, boid& j_boid) { ds += squaresum(i_boid.pos() - j_boid.pos());
      return ds;
  });
  return d_s + local_sum;
  });
  ...
  double stddev = (square_dist_sum / static_cast<double>(boid_vector.size())) -
  (meandist * meandist); */
  double stddev =
      sqrt(sum / (static_cast<double>(boid_vector.size()) *
                  (static_cast<double>(boid_vector.size()) - 1) * 2));
  return stddev;
}  // calcolo della deviazione standard come vuole la sara magica (quadrato
   // della media - media dei quadrati)
